...
const FAMILY_PREFIX = { 'contract.': 'metadata', 'transition.': 'progressCurve', 'transform.': 'transform', 'crop.': 'cropMask', 'shadow.': 'shadow', 'stroke.': 'strokeGlow', 'glow.': 'strokeGlow' };
...
// Per-parameter values needed by native registration (added for the Adobe
// effect). Only numeric/string data the registration call requires is emitted;
// no Adobe SDK types are introduced here. POINT defaults are semantic
// ("frameCenter"/"sourceCenter") and are resolved natively, so defaultNum /
// oldDefaultNum are 0 for POINT (native uses the layer center).
function bindingNumbers(p, contract) {
  const t = p.nativeType;
  ...
  if (t === 'FLOAT_SLIDER' || t === 'ANGLE') {
    out.defaultNum = num(p.default);
    out.oldDefaultNum = num(p.oldProjectDefault);
    out.precision = (t === 'FLOAT_SLIDER' && /durationSeconds|delaySeconds/.test(p.logicalId)) ? 3 : 2;
    if (p.range) {
      out.validMin = num(p.range.min);
      out.validMax = num(p.range.max);
    }
    if (p.uiRange) {
      out.uiMin = num(p.uiRange.min);
      out.uiMax = num(p.uiRange.max);
    }
  } else if (t === 'POPUP') {
    out.defaultNum = num(p.default);
    out.oldDefaultNum = num(p.oldProjectDefault);
...
function emitBindingsHpp(contract, digest) {
  ...
  L.push('  double defaultNum;      // contract default (POINT=0, resolved natively)');
  L.push('  double validMin;        // FLOAT_SLIDER valid range min (else 0)');
  L.push('  double validMax;        // FLOAT_SLIDER valid range max (else 0)');
  L.push('  double uiMin;           // FLOAT_SLIDER UI range min (else 0)');
  L.push('  double uiMax;           // FLOAT_SLIDER UI range max (else 0)');
  L.push('  double oldDefaultNum;   // contract oldProjectDefault (POINT=0)');
  L.push('  int precision;          // FLOAT_SLIDER display decimals (else 0)');
  L.push('  int enumCount;          // POPUP choice count (else 0)');
  L.push('  const char* enumRef;    // POPUP enum name (else "")');
  L.push('};');
  L.push('');
  ...
  L.push(contract.parameters.map((p) => {
    const n = bindingNumbers(p, contract);
    return `  {"${p.logicalId}", ${p.diskId}, "${p.wireName}", "${p.nativeType}", ` +
      `"${p.stateOwnership}", "${p.timeVariance}", ${n.defaultNum}, ${n.validMin}, ${n.validMax}, ` +
      `${n.uiMin}, ${n.uiMax}, ${n.oldDefaultNum}, ${n.precision}, ${n.enumCount}, ${n.enumRef}}`;
  }).join(',\n'));
  L.push('};');
  L.push('');
