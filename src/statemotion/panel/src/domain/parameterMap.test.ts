...
// Run: node --experimental-transform-types src/statemotion/panel/src/domain/parameterMap.test.ts
...
(() => {
  ...
  assert.strictEqual(map.resolve('transition.manualProgress'), undefined);
  pass('missing wireName resolves undefined');
})();

(() => {
  // duplicate wireName: enumerate returns the same index for two wireNames
  const map = new EffectParameterMap((w) => (w === 'SM Mode' ? 0 : 1));
  assert.strictEqual(map.resolve('transition.mode'), 0);
  assert.strictEqual(map.resolve('transition.alignment'), 1); // distinct wireName
  pass('unknown extra param ignored, duplicate wireName first wins');
})();
